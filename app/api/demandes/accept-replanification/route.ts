...
// PUT /api/demandes/accept-replanification - Accepter une replanification (côté propriétaire)
export async function PUT(request: NextRequest) {
  try {
    ...
    if (session.user.role !== "PROPRIETAIRE") {
      return NextResponse.json({ error: "Accès refusé - Seuls les propriétaires peuvent accepter les replanifications" }, { status: 403 });
    }
    ...
    if (updateError) {
      console.error("Erreur lors de l'acceptation de la replanification:", updateError);
      return NextResponse.json({ error: "Erreur lors de l'acceptation de la replanification" }, { status: 500 });
    }
    ...
    console.error("Erreur lors de l'acceptation de la replanification:", error);
    return NextResponse.json(
