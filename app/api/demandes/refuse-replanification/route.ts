...
// PUT /api/demandes/refuse-replanification - Refuser une replanification (côté propriétaire)
export async function PUT(request: NextRequest) {
  try {
    ...
    if (session.user.role !== "PROPRIETAIRE") {
      return NextResponse.json({ error: "Accès refusé - Seuls les propriétaires peuvent refuser les replanifications" }, { status: 403 });
    }
    ...
    // Vérifier que la demande est en statut REPLANIFIEE
    if (demande.statut !== 'REPLANIFIEE') {
      return NextResponse.json({ error: "Cette demande n'est pas en attente de replanification" }, { status: 400 });
    }
    ...
    if (updateError) {
      console.error("Erreur lors du refus de la replanification:", updateError);
      return NextResponse.json({ error: "Erreur lors du refus de la replanification" }, { status: 500 });
    }

    return NextResponse.json({ 
      success: true, 
      demande: updatedDemande,
      message: "Replanification refusée avec succès" 
    });

  } catch (error) {
    console.error("Erreur lors du refus de la replanification:", error);
    return NextResponse.json(
